...
Tips & Tricks:
1. When modifying colours for either your text or your ui themes, a good colour picking tool is essential.  This allows you to look at a proposed colour for some widget and try out variations.  The one I use is: http://www.colorpicker.com.  Although the theme files use hexadecimal to specify colours, you can copy and paste a colour's hexadecimal value from a theme file directly into colorpicker, and it will spit out the colour in both RGB and HSB coordinates.  You can then adjust the colour coordinates until you get exactly what you're looking for.  Each time you make a change, colorpicker will also give you the hexadecimal value.  Once you're satisfied, just copy the hex value and paste it back into the XML tag or the JSON object of the widget you're modifying. 
...
3. If you want to use a custom text colouring scheme in your user preferences, click on Preferences/Settings - User. On the editing panel, enter

    {
        "color_scheme": "Packages/st3-theme-kcps/KCPS_text_theme.xml"
    }

   Unfortunately, Package Control for build 3059 (Spring, 2014) installs custom packages in <data_path>/Installed Packages.  As I noted above, you can manually copy your package into <executable_path>/Packages.  Alternatively, you can place it in <data_path>/Packages. The latter is preferred as the Packages directory in the executables path is, by convention, reserved for packages that ship with ST (see http://www.sublimetext.com/docs/3/packages.html).  Either way, it's the only way to get ST3 to recognize your text theme file, as

    {
        "color_scheme": "st3-theme-kcps/KCPS_text_theme.xml"
    }

will cause an error.  One would prefer this form whereby ST would understand to look in <data_path>/Installed Packages for the package file, open it and discover the referenced theme file within.
 
4. Loose packages, i.e directories named with a package name and containing resource files such as images, themes etc., should live in <data_path>/Packages.
